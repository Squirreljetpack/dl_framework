...
class Data(Base):
    ...
    def loaders(
        ...
        **kwargs: passed to train DataLoader
        ...
        def _loaders(*sets):
            ...
            return tuple(
                td.DataLoader(set, **kwargs)
                for set, kwargs in zip(sets, loader_args)
                if set is not None
            )
        ...
        if isinstance(train_set, td.Dataset):
            ...
            if val_set is not None or split is False:
                return _loaders(self.dataset, val_set)
            else:
    ...
    def preview_batch(self, batch, samples=5, header=True):
        ...
        # Print sample values
        samples = min(samples, len(batch[0]))
        if header:
    ...
    def preview(self, samples=5):
        ...
        if len(loaders) == 2:
            print(f"\nTrain Data ({len(loaders[0])} batches) Preview:")
            print("-" * 50)
            self.preview_batch(next(iter(loaders[0])), samples, header=True)

            if len(loaders) > 1:
                print(f"\nValidation Data ({len(loaders[1])} batches)  Preview:")
                print("-" * 50)
                self.preview_batch(next(iter(loaders[1])), samples, header=False)
        else:
            for i, l in enumerate(loaders):
                print(f"\nLoader {i} ({len(loaders[0])} batches) Preview:")
                print("-" * 50)
